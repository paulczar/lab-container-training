...
<!-- ```execute kubectl wait pod/nginx-without-volume --for condition=ready ``` -->
...
<!-- ```execute kubectl wait pod/nginx-with-volume --for condition=ready ``` -->
...
<!--
```execute kubectl wait pod/nginx-with-git --for condition=initialized```
```execute IP=$(kubectl get pod nginx-with-git -o jsonpath={.status.podIP})```
-->
